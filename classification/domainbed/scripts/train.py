...
if __name__ == "__main__":
    ...
    # Dataloaderの作成．ここでは，envがdatasetを指している．
    ...
    algorithm.to(device)
    # モデルの並列化.
    # DPLのF()など，networkが定義されている場合はモデルの並列化を行う．
    # CLIPなどパラメータ更新を行わない，networkが定義されていない場合は，その子モジュールを並列化
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            # 結果のキー (results_keys) を指定して、表形式で結果を表示するための関数．
            # colwidthは各列の幅を指定するオプションの引数
